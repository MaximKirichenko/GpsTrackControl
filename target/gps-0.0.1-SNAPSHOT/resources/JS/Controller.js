...
function LoadMap(){
  ...
  //Create layers for images
    ...
    var point0 = new OpenLayers.Geometry.Point(29.91, 47.78);
...
function attachFieldsMessage(field){
    ///console.log(field.geometry.bounds.getCenterLonLat().lon);
	$.ajax({
		type : "POST",
		url : 'report/getKadastrInfo',
...
function searchKadNumber(kadNumber){
    ...
    $.getJSON('http://212.26.144.110/kadastrova-karta/find-Parcel',
        ...
        function (data) {
            ...
            console.log("Other hreny: " + data['data'][0]['st_xmin']);
...
function buildPeriodDto() {
	...
	if(timeFrom=="")timeFrom="01:00";
	if(timeTo=="")timeTo="24:00";
	
	console.log("timeFrom" + timeFrom);
	periodDto.dataFrom = parseDate(datepickerFrom, timeFrom);
	periodDto.dataTo = parseDate(datepickerTo, timeTo);
...
function setPoint(data){
	...
	var vectorLastPoint = new OpenLayers.Layer.Vector("Последняя точка",
    		{
    		        styleMap: new OpenLayers.StyleMap(
    		        { "default": stylePoint,
    		          "select": { pointRadius: 20}
