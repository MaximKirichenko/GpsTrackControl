...
public class TerminalDateRepositoryImpl implements TerminalDateRepository {
	...
	@Transactional
	@Override
	public List<TerminalDate> getTerminalDateAboutVehiclesFromPeriod(long millisFrom, long millisTo,
			List<Integer> terminalNumbers) {

		DetachedCriteria getImeisByVehicleIdCrt = DetachedCriteria.forClass(Terminal.class).
		add(Restrictions.in("vehicle", terminalNumbers.toArray(new Integer[terminalNumbers.size()]))).
		...
		List<TerminalDate> dates = sessionFactory.getCurrentSession().
				...
				add(Property.forName("imei").in(getImeisByVehicleIdCrt)).list();
